...
>
> **注意：** 从v1.1.0版本开始，您可以根据不同的target定义不同的`manifest.json`文件。例如，如果需要定义Chrome的专属字段“minimum_chrome_version”，可以在与`manifest.json`相同的目录下创建`manifest.chrome.json`文件并在其中定义该字段。同样地，对于Firefox的专属字段，可以创建`manifest.firefox.json`文件。这些文件的内容将在编译后自动合并并覆盖到构建目录下的`manifest.json`文件中。
>
